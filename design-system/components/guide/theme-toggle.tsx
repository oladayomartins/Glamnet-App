 ...
 *
 * The theme is unknown until after mount, so the first render must match what
 * the server emitted — hence the `mounted` gate and the `dark` default, which
 * is the ThemeProvider's default too. Both the visible label and the accessible
 * name are derived from the same value so they can never disagree.
 */
